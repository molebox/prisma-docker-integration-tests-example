...
import { createOrderNewCustomer, Customer, OrderInput } from '../src/functions/index'

afterAll(async () => {
    await prisma.$disconnect();
});

it('should create 1 new customer with 1 order and 1 invoice', async () => {
    ...
    // Create the order and customer and return the invoice
    const invoice = await createOrderNewCustomer(order);
